...
type Gate_Setting struct {
	...
	URL_SM        *string `json:"url_smart_active"`
}
...
func GetGateSetting(db *sql.DB) (Gate_Setting, bool, error) {
	...
	if err := row.Scan(&data.UGC, &data.IP, &data.Username_gate, &data.Password_gate, &data.URL_SM); err != nil {
...
func SetGateSetting(db *sql.DB, isUse int, IP string, username_gate string, password_gate string, url_smart_active string) (bool, error) {
	log.Printf("Getting setting of gate")
	...
	res, err := stmt.ExecContext(ctx, isUse, IP, username_gate, password_gate, url_smart_active)
	if err != nil {
