...
func SetGateSetting(c *gin.Context) {
	...
	if isSuccess, _ := db_client.SetGateSetting(db, reqBody.Use_gate_checkpoint, reqBody.Url_gate_checkpoint, reqBody.Username_gate, reqBody.Password_gate, reqBody.URL_SM); isSuccess {
		c.JSON(http.StatusOK, gin.H{
			"code":    utils.SUCCESSFULLY_CODE,
			"message": "登録に成功しました。",
		})
