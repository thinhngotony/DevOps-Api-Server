...
namespace Shelf_Register
{
    ...
    {
        OPOS opos;
        public static SimpleTcpClient client_connect;
        ...
        private void init()
        {
            ...
            Session.TcpHost = dataInFile["TcpHost"];
        ...
        private async Task ApiGetDataFromBQ_Sync(string key,string jancode)
        {
            try
            {
                Session.product = new ProductData();
        ...
        private async Task ApiGetImage_Sync(string key, string isbn)
        {
            try
            {
                barcode_state = false;
                ...
                if (content != "")
                {
                    if (data.summary.cover != "")
                    {
                        Session.productPos[key].link_image = data.summary.cover;
                        // Displayed in the user interface
                        if (Session.product.isbn != "")
                        {
                            foreach (PictureBox pictureBox_Items in ImageLayer.Controls.OfType<PictureBox>())
        ...
        private void pictureBox_Click(object sender, EventArgs e)
        {
            ...
            if (!Session.productPos.Keys.Contains("temp"))
            {
                ...
                else if (choosingImage.ImageLocation == "blank_background.png")
                {
                    ...
                    // Ở trên check trùng rồi ở đây check nữa?
        ...
        static bool CreateConnect()
        {
            ...
            {
                // instantiate
                client_connect = new SimpleTcpClient(Session.TcpHost);

                // set events
                client_connect.Events.Connected += Connected;
                client_connect.Events.Disconnected += Disconnected;
                client_connect.Events.DataReceived += DataReceived;
                client_connect.Events.DataSent += Events_DataSent;

                // let's go!
                client_connect.Connect();
                client_connect.Send("ACTION_REGISTER_SHELF_START");
                result = true;
            }
            catch (Exception )
            {
        ...
        //    {
        //        //Send stop singal here
        //        client_connect.Send("ACTION_REGISTER_SHELF_END");
        ...
        //    if (txtLocation.Text != "")
        //    {
        //        Session.time_check = (int)Int64.Parse(txtLocation.Text);
        //    }
